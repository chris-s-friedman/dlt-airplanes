    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Extract all files to a directory
                zip_ref.extractall(destination)# Replace with your desired extraction directory
...
def run_pipeline(month_year, unzip_dir = UNZIP_DIR, file_dir=SPLIT_DIR, fetch=True):
    ...
    if fetch:
        ...
        print(f"Data for {month_year} fetched and split.")
    else:
        print(f"Skipping fetch for {month_year}...")
    ...
    print(f"Pipeline run for {month_year} completed with info: {info}")
...
for month_year in month_year_list:
    ...
    print(f"Completed processing for {month_year}.\n")
